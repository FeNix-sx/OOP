...
сегментов. При этом каждый линейный сегмент предполагается задавать отдельным классом LineTo. Объекты этого
    ...
    def get_length(self):
        ...
        if len(self.path)>1:
            ...
            for ind in range(1, len(self.path)):
                length += sqrt(
                    pow(self.path[ind].x - self.path[ind-1].x, 2) +
                    pow(self.path[ind].y - self.path[ind-1].y, 2)
                )
                print(length)
