...
class DecisionTree:
    @staticmethod
    def __check_tree(obj: TreeObj, x: list):
        if x[obj.indx] == 1:
            return obj.left
        return obj.right
    ...
    def predict(cls, root, x):
        ...
        while True:
            next_obj = cls.__check_tree(root, x)
    ...
    def add_obj(cls, obj: TreeObj, node: TreeObj=None, left=True):
        """
        добавление вершин в решающее дерево (метод должен
        ...
        :param left: флаг, определяющий ветвь дерева (объекта node), к которой присоединяется
